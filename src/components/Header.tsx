        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Menu principal">
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className={`rounded-md px-4 py-2 text-sm font-semibold font-heading transition-colors hover:bg-muted focus-visible:outline-2 focus-visible:outline-primary ${pathname === item.to ? "text-primary" : "text-foreground"
                }`}
            >
              {item.label}
            </Link>
        ...
        <nav className="border-t border-border bg-background px-4 pb-4 md:hidden" aria-label="Menu principal">
          {navItems.map((item) => (
            <Link
              ...
              className={`block rounded-md px-4 py-3 text-base font-semibold font-heading transition-colors hover:bg-muted ${pathname === item.to ? "text-primary" : "text-foreground"
                }`}
            >
              {item.label}
            </Link>
